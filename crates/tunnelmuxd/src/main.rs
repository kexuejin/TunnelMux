...
async fn build_upstreams_health_snapshot(state: &Arc<AppState>) -> UpstreamsHealthResponse {
    let routes = {
        let runtime = state.runtime.lock().await;
        runtime.persisted.routes.clone()
    };
...
mod tests {
    ...
    fn select_route_prefers_host_specific() {
        ...
        let selected =
            select_route(&routes, Some("solomesh.local"), "/chat").expect("route should exist");
